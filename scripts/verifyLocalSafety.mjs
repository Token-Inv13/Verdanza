...
expectScript(
  "verify",
  "npm run verify:local-safety && npm run lint && npm run typecheck && npm run typecheck:cagnotte-interactive && npm run typecheck:api && npm run test:firebase-admin-compatibility && npm run test:firebase-auth-error-contract && npm run test:blog-engagement && npm run test:cagnotte-runtime-config && npm run verify:cagnotte-interactive-isolation && npm run test:cagnotte-interactive-reliability && npm run test:cagnotte-interactive-diagnostics && npm run test:cagnotte-runtime-diagnostics && npm run test:cagnotte-rate-limit-windows && npm run test:cagnotte-interactive && npm run test:cagnotte-admin-storage && npm run test:cagnotte-admin-ui && npm run test:cagnotte-emulator-diagnostics && npm run test:cagnotte-read && npm run test:order-refunds && npm run test:cagnotte-payment-links && npm run test:cagnotte-security && npm run test:cagnotte-checkout-use && npm run test:referral-backend && npm run test:cagnotte-v1-recipe && npm run verify:cagnotte-production-readiness && npm run test:core && npm run build:local && npm run audit:local-essential",
);
expectScript(
  "verify:full",
...
expectScript(
  "test:cagnotte-interactive-reliability",
  "node --import tsx scripts/cagnotte-interactive/testReliability.ts",
);
expectScript(
  "test:cagnotte-interactive-diagnostics",
  "node --import tsx scripts/cagnotte-interactive/testDiagnosticJournal.ts",
);
expectScript(
  "test:cagnotte-runtime-diagnostics",
  "node --import tsx scripts/cagnotte-interactive/testRuntimeDiagnostics.ts",
);
expectScript(
  "test:cagnotte-rate-limit-windows",
  "node --import tsx scripts/runCagnotteLedgerTests.ts --rate-limit-windows-only",
);
expectScript(
...
expectScript("test:firebase-auth-error-contract", "node --import tsx scripts/testFirebaseAuthErrorContract.ts");
expectScript(
  "test:cagnotte-v1-recipe",
  "node --import tsx scripts/runCagnotteLedgerTests.ts --v1-recipe-only && node --import tsx scripts/captureCagnotteV1Recipe.ts",
);
...
requireValue(
  ...
  ].every((command) => !isExactOrderRefundPreparedEmulatorCommand(command)),
  "test:order-refunds refuse les chaînes avec préparation réseau implicite, tardive ou incorrecte",
...
expectScript(
  "build:local",
  "npm run sitemap:check && tsc -b && vite build && npm run prerender && npm run verify:cagnotte-normal-build",
);
...
function expectOrderRefundPreparedEmulator(command) {
  requireValue(
    isExactOrderRefundPreparedEmulatorCommand(command),
    "test:order-refunds utilise l émulateur préparé sans téléchargement implicite",
    "test:order-refunds doit exécuter directement le runner refunds avec un émulateur déjà préparé",
  );
}

function isExactOrderRefundPreparedEmulatorCommand(command) {
  return command === "node --import tsx scripts/runCagnotteLedgerTests.ts --refunds-only";
}
