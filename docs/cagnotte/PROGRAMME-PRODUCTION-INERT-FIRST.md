...
Les preuves courantes sont indexées par `node_modules/.cache/verdanza-cagnotte-interactive/latest-result.json`. Ce fichier est supprimé au début de chaque nouvelle exécution et n'est recréé qu'après une réussite complète ; un échec écrit `latest-failure.json`. Chaque sous-dossier de `node_modules/.cache/verdanza-cagnotte-interactive/runs/` contient les captures, le relevé réseau navigateur expurgé, la console, la sonde Listen, un résumé même interrompu, le résultat du nettoyage, les requêtes API et les états Firestore du parcours. La CI conserve seulement une liste explicite de ces diagnostics sûrs et de cinq captures utiles ; elle n'archive ni profil navigateur, ni cache complet, ni logs Auth bruts.

### Validation et portée

La validation obligatoire inclut désormais le typecheck dédié, les contrôles statiques d'isolation, les tests d'échec injecté du cycle de vie et de classification, le parcours interactif et un contrôle du build normal qui refuse tout marqueur ou module de recette. Les ressources sont enregistrées dès leur acquisition ; chaque nettoyage est borné, tente les autres fermetures après un échec, conserve l'erreur initiale et accepte un second arrêt sans tuer de processus par nom ou par port. La CI prépare séparément Chromium et les émulateurs avant `npm run verify`, sans téléchargement implicite pendant cette commande. Les sept gardes normales restent :
