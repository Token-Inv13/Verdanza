...
export function ProductRecommendation({ matches, selectionKey }: { matches: ProductSheetMatch[]; selectionKey: string }) {
  const [promotion, setPromotion] = useState<{ selectionKey: string; slug: string } | null>(null);
  const viewedSignatureRef = useRef("");
  const detailsRef = useRef<HTMLDivElement>(null);
  const promotedSheet = promotion?.selectionKey === selectionKey ? matches.find((match) => match.sheet.slug === promotion.slug) : undefined;
  const activeMatch = promotedSheet || matches[0];
  const alternatives = matches.filter((match) => match.sheet.slug !== activeMatch?.sheet.slug).slice(0, 2);
            ...
            <div className="order-2 mt-5 grid w-full grid-cols-2 gap-3 md:absolute md:inset-0 md:order-none md:mt-0 md:block md:pointer-events-none">
              {alternatives.map((match, index) => (
                <button
                  ...
                  onClick={() => setPromotion({ selectionKey, slug: match.sheet.slug })}
                  aria-label={`Afficher ${match.sheet.name} comme profil principal`}
                  data-selector-alternative={match.sheet.slug}
                >
                  <img src={match.sheet.previewUrl} alt="" width={640} height={888} loading="lazy" decoding="async" className="aspect-[111/154] w-full object-cover" />
                  <span className="block border-t border-forest/10 px-3 py-3">
                    <span className="block text-[0.65rem] font-semibold uppercase tracking-[0.12em] text-champagne">Autre profil</span>
                    <span className="mt-1 block text-sm font-semibold text-forest">{match.sheet.name}</span>
                  </span>
                </button>
              ))}
            </div>

            <TiltProductCard key={`${selectionKey}:${activeMatch.sheet.slug}`} sheet={activeMatch.sheet} onOpenDetails={() => detailsRef.current?.focus({ preventScroll: true })} />
          </div>
        </div>

        <div ref={detailsRef} id="product-selector-result-details" className="min-w-0 outline-none" tabIndex={-1} data-selector-result-details>
          ...
          <h4 className="mt-2 font-display text-4xl leading-tight text-forest">{activeMatch.sheet.name}</h4>
          <p className="mt-3 text-base leading-7 text-ink/65">{activeMatch.sheet.aromas.join(" · ")}</p>

          <dl className="mt-7 max-w-xs">
            <ResultDetail label="Intensité">{productSheetIntensityLabels[activeMatch.sheet.selectionProfile.intensity]}</ResultDetail>
          </dl>
          ...
          {alternatives.length > 0 && (
            <div className="mt-8 border-t border-forest/10 pt-6">
              <p className="text-xs font-semibold uppercase tracking-[0.17em] text-forest/55">Autres profils</p>
              <p className="mt-2 text-sm leading-6 text-ink/55">Sélectionnez une carte du deck pour l’afficher au premier plan.</p>
            </div>
          )}
        </div>
      </div>
...
function TiltProductCard({ sheet, onOpenDetails }: { sheet: ProductSheet; onOpenDetails: () => void }) {
  ...
  return (
    ...
    >
      ...
      <span className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-forest/95 via-forest/80 to-transparent px-5 pb-5 pt-16 text-ivory">
        <span className="block text-[0.65rem] font-semibold uppercase tracking-[0.15em] text-champagne">Correspond à votre sélection</span>
...
function ResultDetail({ label, children }: { label: string; children: string }) {
  return (
    <div className="rounded-lg border border-forest/10 bg-ivory p-4">
      <dt className="text-[0.7rem] font-semibold uppercase tracking-[0.16em] text-forest/55">{label}</dt>
