...
export function ProductProfileSelector() {
  ...
  const startSelector = () => {
    ...
    trackEvent("product_selector_started", { selector_version: "v6" });
  };

  const selectCategory = (category: ProductSheetCategory) => {
    startSelector();
    setChoices((current) => changeProductSelectorCategory(current, category));
    setOpenStep(2);
    ...
    >
      <div className="grid gap-7 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
        <div className="max-w-3xl">
          ...
          >
            Trouvez votre fiche en trois choix
          </h2>
          <p className="mt-4 text-base leading-7 text-ink/65 sm:text-lg">
            Choisissez un type, une intensité, puis affinez simplement par arômes si vous le souhaitez.
          </p>
        </div>
        <div className="flex flex-wrap gap-3 lg:justify-end">
          ...
          <a href="#toutes-les-fiches" className="btn-secondary min-h-11 px-4 py-2 text-sm">
      ...
      <ol
        ...
        aria-label="Progression du sélecteur : Type, Intensité, Arômes"
      >
          ...
          <OptionGrid>
            {intensityOptions.map(([value, label]) => (
              <ChoiceOption
                ...
                label={label}
                description={
                  choices.category && !availableIntensities.has(value)
                ...
                selected={
                  availableIntensities.has(value) && choices.intensity === value
                }
      ...
      <p className="mt-5 text-xs leading-5 text-ink/50">
        Le type et l’intensité filtrent strictement la sélection. Les arômes ne font qu’en affiner l’ordre.
      </p>

      {matches.length > 0 && (
        <ProductRecommendation matches={matches} selectionKey={selectionKey} />
      )}
      {requiredSelectionComplete && matches.length === 0 && (
        <p className="mt-8 rounded-lg border border-forest/10 bg-ivory p-5 text-sm leading-6 text-forest" aria-live="polite" data-product-selector-empty>
          {choices.category === "resin" && choices.intensity === "douce" ? (
            <>Aucun produit ne correspond encore à cette intensité. Essayez Moyen ou Fort.</>
          ) : (
            <>Aucun produit ne correspond exactement à ce type et à cette intensité.</>
          )}
        </p>
      )}
    </section>
...
function SelectorStep({
  ...
  return (
    <div
      className={`selector-step rounded-lg border bg-ivory transition ${open ? "border-champagne/55 shadow-sm" : "border-forest/10"} ${locked ? "opacity-55" : ""}`}
    ...
    >
      ...
      >
        <span className={`flex h-9 w-9 items-center justify-center rounded-full border text-xs font-semibold ${completed ? "border-forest bg-forest text-ivory" : "border-champagne/55 bg-cream text-forest"}`} aria-hidden="true">
          {completed ? <Check size={14} /> : number}
        </span>
        <span className="min-w-0">
          <span className="flex flex-wrap items-center gap-2 text-sm font-semibold">
            {title}
            {optional && <span className="text-[0.65rem] font-medium uppercase tracking-[0.12em] text-forest/45">Facultatif</span>}
          </span>
          <span className="mt-1 block truncate text-xs font-medium text-ink/50">{summary}</span>
        </span>
        <ChevronDown aria-hidden="true" size={18} className={`transition-transform duration-200 ${open ? "rotate-180" : ""}`} />
      </button>
      <fieldset id={panelId} className={`selector-step-content m-0 min-w-0 border-0 p-0 ${open ? "is-open" : ""}`} aria-hidden={!open} disabled={!open}>
        <div><div className="border-t border-forest/10 px-4 pb-5 pt-4 sm:px-5">{children}</div></div>
      </fieldset>
    </div>
  );
}
...
function TypeOption({ category, title, description, selected, onSelect, icon }: { category: ProductSheetCategory; title: string; description: string; selected: boolean; onSelect: (category: ProductSheetCategory) => void; icon: ReactNode }) {
  return (
    <button
      type="button"
      className={`group min-h-28 rounded-lg border p-5 text-left transition focus:outline-none focus:ring-2 focus:ring-champagne focus:ring-offset-2 ${selected ? "border-forest bg-forest text-ivory" : "border-forest/15 bg-cream/55 text-forest hover:border-champagne"}`}
      onClick={() => onSelect(category)}
      aria-pressed={selected}
      data-selector-option={`category:${category}`}
    >
      <span className="text-champagne">{icon}</span>
      <span className="mt-5 block text-sm font-semibold uppercase tracking-[0.15em]">{title}</span>
      <span className={`mt-2 block text-xs leading-5 ${selected ? "text-ivory/70" : "text-ink/55"}`}>{description}</span>
    </button>
  );
}
...
function ChoiceOption({
  ...
  return (
    <button
      type="button"
      className={`min-h-11 rounded-md border px-4 py-2.5 text-sm font-semibold uppercase tracking-[0.06em] transition focus:outline-none focus:ring-2 focus:ring-champagne focus:ring-offset-2 ${
        disabled
    ...
    >
      ...
      {description && (
        <span className="mt-1 block text-[0.65rem] font-medium normal-case tracking-normal">
          {description}
        </span>
      )}
    </button>
